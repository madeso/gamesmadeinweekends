...
function World:onRandom(Type, Callback)
	local done = false
	local id = 0
	local o = 0
	repeat
		id = math.random(#self.objects)
		o = self.objects[id]
		--print(id, o)
		if o:is_a(Type) then
			Callback(o)
...
function World:forEach(Type, Callback)
...
function isRemoveSet(object)
	if object.removeThis then
		if object.leaveWorld then
			object:leaveWorld()
		end
		return true
	else
...
function World:update(dt)
	...
	remove_if(self.objects, isRemoveSet)
end
...
function World:onkey(down, key, unicode)
	...
	for i,o in ipairs(self.objects) do
		o:onkey(down, key, unicode, world)
	end
end